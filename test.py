...
def serialize_request(record: dict) -> dict:
    # Convert datetime / enum to serializable forms
    out = dict(record)
    for key in ['vytvorene', 'aktualizovane']:
        if key in out and out[key] is not None:
            out[key] = out[key].isoformat()
    enum_keys = ['typ_pouzivatela', 'kategoria', 'status']
...
async def vytvorit_ziadost(data: ChatInput):
    """
    ...
    await manager.broadcast({"event": "new_request", "request": serialize_request(record)})
...
async def aktualizovat_status(request_id: str, update: StatusUpdate):
    """Zmení stav žiadosti."""
    record = get_request_or_404(request_id)
